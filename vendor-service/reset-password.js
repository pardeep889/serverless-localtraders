...
const { getToken, decodeToken } = require("./token");
...
module.exports.handler = async (request) => {
  ...
  try {
    ...
    const paramsScan = {
      ...
      FilterExpression: "email = :email and passwordToken = :passwordToken",
    ...
    if (data.Items.length > 0) {
      ...
      await dynamoDb
        .update({
          TableName: utils.TABLE_NAME,
          Key: { id },
          UpdateExpression: "set #password = :password, #passwordToken = :passwordToken",
          ExpressionAttributeValues: {
