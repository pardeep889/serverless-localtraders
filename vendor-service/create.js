...
module.exports.handler = async (event) => {
  ...
  let vendor;
  try {
    vendor = JSON.parse(event.body);
  } catch (err) {
    return utils.send(400, {
      message: 'request vendor is not a valid JSON',
    });
      }

  if (!vendor.email || !vendor.firstName|| !vendor.lastName || !vendor.password) {
  ...
  let { firstName, lastName, email, password } = vendor;
  ...
  try {
    ...
    const { password: pwd, ...resp } = VendorData;

    // sent verification email
   
    const host = `https://q3j8v1plql.execute-api.us-east-1.amazonaws.com/vendor/email/verify/${token}`;
    ...
    return utils.send(200, {
      data: { ...resp, token, isEmailSend : emailResponse?.messageId ? true :false },
    });
  } catch (error) {
