...
async function updateBalance(assetId, updatedBalance) {
    try {
      ...
      const updateResp =  await dynamoDb.update(paramsUpdate).promise();
...
async function retrieveRecipientBalance(recipient_id) {
  // Code to retrieve recipient's balance from database goes here
}

async function updateRecipientBalance(recipient_id, updatedRecipientBalance) {
  // Code to update recipient's balance in database goes here
}
...
module.exports.handler = async (request) => {
  try {
    ...
    const senderResp = await retrieveAssetBalance(
      sender_id,
      symbol
    );
    ...
    const receipientResp = await retrieveAssetBalance(
        recipient_id,
        symbol
      );
      ...
      const updated_balance_of_receiver = parseFloat(receipientResp?.balance) + amount;
      console.log({updated_balance_of_receiver})

      // update balance for sender and receiver
      updateBalance(senderResp.assetId,updated_balance_of_sender)
      updateBalance(receipientResp.assetId,updated_balance_of_receiver)
